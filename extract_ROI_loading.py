...
data_dict={}
for gene_id,label in zip(np.asarray(labels_df.get('id'))[1:],np.asarray(labels_df.get('atlas_id'))[1:]): #iterating through the ROI ids
    if np.max(label==atlas_data): #taking a ROI only if it has labeled voxels
        ...
        data_dict[str(gene_id)]=ROI_loading #assigning the ROI loading to the corresponding ROI id in the dictionary
        print(label)
